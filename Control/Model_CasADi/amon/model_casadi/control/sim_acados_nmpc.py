...
def main():
    ...
    x_rollout = x.copy()
    solver.set(0, "x", x_rollout)
    for stage in range(N):
        def ode(_, x_):
            return np.array(f(x_, u_prev)).reshape(-1)

        sol = solve_ivp(ode, [0, dt], x_rollout, method="RK45", max_step=dt / 4)
        x_rollout = sol.y[:, -1]
        solver.set(stage + 1, "x", x_rollout)
    ...
    plt.tight_layout()
    output_dir = AMON_ROOT / "model_casadi" / "acados_generated" / "output"
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(output_dir / "acados_sim.png", dpi=150, bbox_inches="tight")
    plt.close()
